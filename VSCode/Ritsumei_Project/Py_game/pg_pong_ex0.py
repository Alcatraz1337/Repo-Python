...
for i in range(0, balls):
    ball.append(MovingSphere(randint(0, screen.get_width()), 0,ball_radius, temp_ball_speed_x, ball_speed_y, random_color()))
    temp_ball_speed_x = -temp_ball_speed_x

# loads the R image (must be saved in the same place as this file)
my_R = pygame.image.load(r'Ritsumei_Project\Py_game\Rh.jpg')
# generates the paddle
paddle = MovingRectangle(screen.get_width()/2, screen.get_height()-2 *my_R.get_height(), my_R.get_width(), my_R.get_height(), 0, 0, [0, 0, 0])
